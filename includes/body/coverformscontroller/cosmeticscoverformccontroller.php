<?php
if(isset($_POST['postcosmeticcover']))
{
    ...
    else
    {
        ...
        if (in_array($file_extn, $allowed) === true)
        {
                ...
                $stmt = $auth_user->runQuery("SELECT user_id, user_name FROM cosmeticscover WHERE user_id=:user_id ");
                ...
                if($row['user_id']==$user_id) {
                    echo "Found"; 
                        ...
                        try
                        {
                            ...
                            $file_path = 'uploads/covercosmeticimages/' . substr(md5(time()), 0, 20) . '.' . $file_extn;
                            move_uploaded_file($file_tmp, $file_path);

                            if($auth_user->addcosmeticcover($user_id, $user_name, $file_path, $backgroundcolor))
                            {
                                ?>
